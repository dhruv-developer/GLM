...
class SchedulerAgent(BaseAgent):
    ...
    async def execute(self, action: str, parameters: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            if action == "schedule_task":
                # Generic schedule_task action generated by the controller
                ...
                logger.warning(f"Scheduler: unknown action '{action}', returning failed")
    ...
    async def _schedule_task(self, params: Dict[str, Any]) -> Dict[str, Any]:
        """Handle generic schedule_task action from the controller planner"""
        time_info = params.get("time_info", {})
        task_data = params.get("task_data", "")

        # Validate that time_info is present and not empty
        if not time_info or (isinstance(time_info, dict) and not time_info):
            return self._create_response(
                status="failed",
                error="Time information is required for scheduling"
            )

        logger.info(f"Scheduling task: {task_data}")
